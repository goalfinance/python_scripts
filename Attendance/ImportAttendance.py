...
def main():
    ...
    if const.APP_PARAMS_SOURCE_FILE not in app_params:
        print("The source file should not be absent, please assigning it by using '-s' or '--source'")
        sys.exit(2)

    if const.APP_PARAMS_TARGET_FILE not in app_params:
    ...
    try:
        ...
        trans_date = None
        if const.APP_PARAMS_MONTH not in app_params or app_params[const.APP_PARAMS_MONTH] == None:
